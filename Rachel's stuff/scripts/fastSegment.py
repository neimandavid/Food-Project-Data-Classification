...
def get_gyro_df(imu_df,train=False): #Raw gyro data
    ...
    columns_sel=['secs','nsecs','z','y.1','wDist','Label']
    gyro_df = imu_df[columns_sel]
    
    gyro_df=gyro_df.rename(index=str, columns={"z": "first","y.1": "second", "wDist":"third", 'secs':'Seconds','nsecs':'NanoSeconds', 'Label': 'label'})
    r = len(imu_df)
    time = []
