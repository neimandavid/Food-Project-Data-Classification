...
HMMdata = zeros(nclumps, nclumps);

filepath = strcat(modeltouse, '/');
filepath = strcat(filepath, 'HMM/');
...
for filetoread = filestoread
    ...
    for i = 2:N
        HMMdata(indicator(i-1), indicator(i)) = HMMdata(indicator(i-1), indicator(i)) + 1; %Seriously, why does Matlab not have a ++ operator?
    end
end
